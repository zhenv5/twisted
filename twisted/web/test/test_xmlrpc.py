...
from twisted.python.compat import (
    nativeString, networkString, NativeStringIO, intToBytes)
...
class XMLRPCTests(unittest.TestCase):
    ...
    def test_headers(self):
        ...
        def check_server_headers(ing):
            self.assertEqual(
                self.factories[0].headers[b'content-type'], b'text/xml')
            self.assertEqual(
        ...
        def check_client_headers(ign):
            self.assertEqual(
                ...
                b'Twisted/XMLRPClib')
            self.assertEqual(
                self.factories[0].sent_headers[b'content-type'], b'text/xml')
            self.assertEqual(
...
class XMLRPCClientErrorHandlingTests(unittest.TestCase):
    ...
    """
    def setUp(self):
        class ResourceWithInvalidResponse(Resource.Resource):
            isLeaf = True
            def render_GET(self, request):
                data = b"This text is not a valid XML-RPC response."
        ...
        self.port = reactor.listenTCP(
            0, server.Site(ResourceWithInvalidResponse()),
            interface='127.0.0.1')
